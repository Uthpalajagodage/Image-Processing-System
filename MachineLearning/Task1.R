...
#Perform k-means clustering on the pre-processed data
  ...
  par(mar=c(1,1,1,1))
  k <- elbow
  kmeans_fit <- kmeans(transformed_data, k)
  ...
  BSS_transf <- sum(kmeans_fit$size * apply((kmeans_fit$centers - apply(transformed_data, 2, mean))^2, 1, sum))
  TSS_transf <- sum(apply(transformed_data^2, 1, sum)) - (sum(transformed_data)^2)/length(transformed_data)
  WSS_transf <- kmeans_fit$tot.withinss
